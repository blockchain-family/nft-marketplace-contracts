// @ts-check
import {deployCollection} from "../test/utils";
...
const { deployCollection, deployAccount, deployTokenRoot, getAccount, Contract, LockLift, getRandomNonce, isValidTonAddress, logContract, getTotalSupply } = require('../test/utils')
  ...
  const response = await prompts([
    {
      type: 'text',
      name: 'owner',
      message: 'Get Collection Owner Address',
    ...
    try {
      ...
      for (let i = 0; i < amount; i += INCREMENT) {
        ...
        let tx = await tempAdmin.runTarget({
          contract: collection,
          method: 'batchMintNft',
          params: { owner: config.owner, jsons: jsons },
          keyPair,
