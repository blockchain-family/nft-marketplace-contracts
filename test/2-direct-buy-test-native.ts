import {
    ...
    deployAuctionRoot, deployWeverRoot
} from "./utils";
   ...
   it('Mint TIP-3 token to account', async function () {
    ...
    describe("DirectBuy completed", async function () {
        it('Deploy limited DirectBuy and success', async function () {
            ...
            await tokenWallet3.transfer(spentToken, factoryDirectBuy.address, toNano(0.2), true, payload, toNano(5));
            ...
            const { traceTree } = await locklift.tracing.trace(nft.changeManager(account2, directBuy.address, account2.address, callbacks));
        ...
        //     await tokenWallet2.transfer(spentToken, factoryDirectBuy.address, toNano(0.2), true, payload,toNano(5));
        //
        ...
        //     const spentToken: number = 1000000000;
        //     let payload: string;
        //     payload = (await factoryDirectBuy.buildPayload(0, account3, nft, Math.round(Date.now() / 1000), 8));
    ...
    describe("Change market fee for direct buy", async function () {
        it('Change market fee and success buy', async function () {
            ...
            console.log('expectedAccountBalance',expectedAccountBalance);
            console.log('everAccount3Balance',everAccount3Balance);
            // expect(everAccount3Balance).to.be.closeTo(expectedAccountBalance, 0.1);
